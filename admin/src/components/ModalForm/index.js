...
const ModalForm = (props) => {
  ...
  const { onSubmit, contentTypes, onChange, onCancel, settingsType } = props;
  ...
  const handleSelectChange = (e, uidFields) => {
    ...
    if (uidFields.length === 1) {
      setState((prevState) => ({
        ...prevState,
        selectedUidField: uidFields[0],
      }));
      onChange(
        { target: { name: "uidField", value: uidFields[0] } },
        contentType,
        settingsType
      );
    }
  ...
  if (!isEmpty(edit)) {
    ...
    if (settingsType === "Collection") {
      uidFields = getUidFieldsByContentType(
        contentTypes.filter(
          (mappedContentType) => mappedContentType.apiID === edit
        )[0]
      );
      ...
      <HeaderModal>
        <section style={{ alignItems: "center" }}>
          ...
          <HeaderModalTitle style={{ marginLeft: 15 }}>
            {globalContext.formatMessage({ id: "sitemap.Modal.HeaderTitle" })} -{" "}
            {settingsType}
          </HeaderModalTitle>
        </section>
      </HeaderModal>
          ...
          <section style={{ marginTop: 20 }}>
            ...
            {isEmpty(edit) && (
              <p style={{ maxWidth: 500 }}>
                {settingsType &&
                  globalContext.formatMessage({
                    id: `sitemap.Modal.${settingsType}Description`,
                  })}
              </p>
            )}
            <form
              ...
              <div className="col-md-6">
                {settingsType === "Collection" ? (
                  <SelectContentTypes
                    ...
                    onChange={(e, uidFields) =>
                      handleSelectChange(e, uidFields)
                    }
                    value={contentType}
                  ...
                  <InputUID
                    ...
                    value={contentType}
                    label={globalContext.formatMessage({
                      id: "sitemap.Settings.Field.URL.Label",
                    })}
                    description={globalContext.formatMessage({
                      id: "sitemap.Settings.Field.URL.Description",
                    })}
                    name="url"
                    disabled={!isEmpty(edit)}
                  />
                  ...
                  {settingsType === "Collection" && (
                    <div className="col-12">
                      <InputUID
                        ...
                        label={globalContext.formatMessage({
                          id: "sitemap.Settings.Field.Area.Label",
                        })}
                        description={globalContext.formatMessage({
                          id: "sitemap.Settings.Field.Area.Description",
                        })}
                        name="area"
